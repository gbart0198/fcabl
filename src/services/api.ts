...
import type { TeamDetail, Standing, PlayerProfile, GameWithDetails, TeamWithStats } from '@/types/game.types';
...
import { combineDateTime } from '@/utils/game';
...
export const teamService = {
   ...
   * @returns Promise resolving to array of teams with stats
   */
  async getAllTeams(): Promise<TeamWithStats[]> {
    await new Promise(resolve => setTimeout(resolve, 300));
    return Object.values(mockTeamDetails).map(td => teamToTeamWithStats({
      id: td.id,
      name: td.name,
      wins: td.wins,
      losses: td.losses,
      draws: td.draws,
      pointsFor: td.pointsFor,
      pointsAgainst: td.pointsAgainst,
...
export const scheduleService = {
   ...
   * @returns Promise resolving to array of all games with details
   */
...
export const adminService = {
  ...
  async updateTeam(id: string, data: UpdateTeamData): Promise<TeamDetail | null> {
    ...
    team.name = data.name;
    team.updatedAt = new Date().toISOString().slice(0, 19);
    
    // Update team name in all games
    mockAllGames.forEach(game => {
      if (game.homeTeamId === id) game.homeTeamName = data.name;
      if (game.awayTeamId === id) game.awayTeamName = data.name;
    });
   ...
   * @returns Promise resolving to the created game with details
   */
  async createGame(data: CreateGameData): Promise<GameWithDetails> {
    await new Promise(resolve => setTimeout(resolve, 300));
    ...
    const timestamp = new Date().toISOString().slice(0, 19);
    const newGame: GameWithDetails = {
      id: newId,
      homeTeamId: data.homeTeamId,
      awayTeamId: data.awayTeamId,
      homeTeamName: homeTeam.name,
      awayTeamName: awayTeam.name,
      gameTime: combineDateTime(data.date, data.time),
      status: data.status,
      createdAt: timestamp,
      updatedAt: timestamp,
    };
   ...
   */
  async updateGame(id: string, data: UpdateGameData): Promise<GameWithDetails | null> {
    await new Promise(resolve => setTimeout(resolve, 300));
    ...
    const updatedGame: GameWithDetails = {
      ...existingGame,
      gameTime: data.date && data.time ? combineDateTime(data.date, data.time) : existingGame.gameTime,
   ...
   */
  async getUnassignedPlayers(): Promise<PlayerProfile[]> {
    await new Promise(resolve => setTimeout(resolve, 300));
    return mockUnassignedPlayers;
  },
   ...
   */
  async getTeamPlayers(teamId: string): Promise<PlayerProfile[]> {
    await new Promise(resolve => setTimeout(resolve, 300));
  ...
  async assignPlayerToTeam(playerId: string, teamId: string, number?: number): Promise<{ success: boolean; error?: string }> {
    ...
    const player = mockUnassignedPlayers[playerIndex];
    ...
    player.isFullyRegistered = true;
    player.registrationFeeDue = null;
  ...
  async removePlayerFromTeam(playerId: string, teamId: string): Promise<{ success: boolean }> {
    ...
    player.registrationFeeDue = 50.00;
    player.isFullyRegistered = false;
